...
// special rendering of first updates to visualize the spin
		if (fjonga++ < 8) {
			...
			Visualizer.broadcastMessage(VisualMessageTool.updateMessage("line", "futuretunkki" + fjonga, "class", "pretr", "x1", now.x, "y1", now.y, "x2", next.x, "y2", next.y, "style", "stroke: green; stroke-opacity: 0.3;").toString());
		}

		for (int i = 0; i < 7; i++) {
			BallStatus next = calculateNext(now);
			Visualizer.broadcastMessage(VisualMessageTool.updateMessage("line", "future" + i, "class", "pretr", "x1", now.x, "y1", now.y, "x2", next.x, "y2", next.y).toString());
			now = next;
		}
